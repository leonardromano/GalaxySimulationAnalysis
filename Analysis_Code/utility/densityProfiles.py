...
def createDensityProfileHistogramm(masses, radii, logrmin, logrmax, dlogr, unit):
    ...
    getLocalDensity(densityArray, logrmin, dlogr)
    radiusArray = np.asarray(xValues)
    logrho = np.log10(densityArray)
    np.savetxt('densityProfile', getXYArray(radiusArray, logrho), delimiter = ' ')
    return getXYArray(radiusArray, logrho)

def getLocalDensity(densityArray, logrmin, dlogr):
    "Prints the local Density"
    nsun = 2*int(round((np.log10(8.122)-logrmin)/dlogr))
    print "rho(" + str((nsun/2-1)*dlogr) +") = " + str(densityArray[nsun-2]) + " A.U.\n" + \
    "rho(" + str(nsun*dlogr/2) +") = " + str(densityArray[nsun]) + " A.U. \n" + \
    "rho(" + str((nsun/2+1)*dlogr) +") = " + str(densityArray[nsun+2]) + " A.U. \n"
